...
public class Client {
	...
	private static ArrayList<MessageBasedSensorModel> sensorValues = new ArrayList<MessageBasedSensorModel>();
	private static ArrayList<Action> actuatorValues = new ArrayList<Action>();
	...
	public static boolean runClient(String[] args, ParametersContainerE6 para) throws IOException {
			...
			while (true) {
				...
				if (inMsg != null) {
					...
					if (currStep < maxSteps || maxSteps == 0){
						...
						MessageBasedSensorModel sensorInput = new MessageBasedSensorModel(inMsg);
						//action = new Action();
						action = driver.control(sensorInput);
						sensorValues.add(sensorInput);
						action.limitValues();
						//System.out.println(action.getAllValuesAsString());
						actuatorValues.add(new Action(action));
					}
					else
		...
		driver.shutdown();
		mySocket.close();
//		System.out.println("Sensors length: " + sensorValues.size());
//		System.out.println("Actuators length: " + actuatorValues.size());
		return true;
	}
	...
	private static void writeDataToCSV() throws IOException {
		...
		File dir = new File("..\\..\\data\\"+trackName);
		if (!dir.exists()) {
			dir.mkdirs();
		}
		
		File csvFile = new File(dir, "\\"+raceNumber+".csv");
		...
		if (csvFile.exists()) {
			...
			writer.write(MessageBasedSensorModel.getHeaderString());
			writer.write(Action.getHeaderString() + "\n");
		}
		
		for (int i = 0; i < sensorValues.size(); i++) {
			writer.write(sensorValues.get(i).getAllValuesAsString());
			writer.write(actuatorValues.get(i).getAllValuesAsString() + "\n");
		}
